...
from writing import set_writer_prompt
import Prompts


def initialize_chosen_model(model: str, key_or_path: str) -> BaseModelBackend:
    """
    Initializes the specified model using Camel-AI's ModelFactory.
    Validates the model name against the external Model enum.

    Args:
        model (str): Name of the model to initialize.
        key_or_path (str): API key for proprietary models or path to Ollama setup.

    Returns:
        BaseModelBackend: Fully initialized model suitable for agent initialization.

    Raises:
        ModelNotFoundError: If the model is unknown or missing.
        RuntimeError: If required keys or paths are missing.
    """
...
def initialize_brainstorming_agents(model:BaseModelBackend) -> dict[ChatAgent]:
    """
    Initializes the agents required for the brainstorming phase using the provided model.

    Args:
        model (BaseModelBackend): The model used to initialize the agents.

    Returns:
        dict[str, ChatAgent]: Dictionary containing initialized ChatAgent instances:
            - "planner": ChatAgent responsible for planning story elements.
            - "critic": ChatAgent responsible for providing feedback on generated content.
    """
...
def initialize_writing_agents(model:BaseModelBackend, story_glossary: StoryGlossary, scene_nr: int) -> dict[ChatAgent]:
    """
    Initializes the agents required for the writing phase using the provided model.

    Args:
        model (BaseModelBackend): The model used to initialize the agents.
        story_glossary (StoryGlossary): The StoryGlossary object created during brainstorming.
        scene_nr (int): Number of scenes to be written (used for context if needed).

    Returns:
        dict[str, ChatAgent]: Dictionary containing initialized ChatAgent instances:
            - "taskmaster": TaskPlannerAgent for planning scenes.
            - "writer": ChatAgent for generating scene content.
            - "feedback": ChatAgent for providing critic feedback.
    """
